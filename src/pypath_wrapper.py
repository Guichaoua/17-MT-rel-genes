...
class Wrap_net(Network):
    ...
    def write_df(self):
        if self.interactions == {}:
            print("there are no interaction in this network")
            return pd.DataFrame()
        ...
        for node in list(self.nodes_by_label.keys()):
            ...
            L_on = [[node, g, 1] for g in formula_on]
            L_off = [[node, g, -1] for g in formula_off]
            L+= L_on + L_off

        return pd.DataFrame(L,columns=['node1','node2','interactions'])
